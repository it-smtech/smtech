...
class GameContainer extends React.Component {
  constructor(props) {
    super(props);
    console.log("Dans GameContainer roomNumber = " + this.props.roomNumber);
    this.state = {
  ...
  componentDidMount() {
    this.state.client.on("updateRoomUsers", (message) => {
      console.log("got reply getUserID GameContainer", message);
      let player = new Player(message.userName);
      console.log("state = " + this.state);
      this.setState((state) => ({
        players: [...state.players, player],
      }));
      console.log("got reply users", this.state.players);
    });
  }
  startGame() {
    if (this.playerNames.length < 2) {
      ...
      this.setState({
        chanceCards: this.shuffle(this.state.chanceCards),
  ...
  updateActivePlayer() {
    if (this.state.rolled) {
      this.setState({
        activePlayer:
          this.players[
            (this.state.activePlayerIndex + 1) % this.players.length
          ],
        activePlayerIndex:
          (this.state.activePlayerIndex + 1) % this.players.length,
  ...
  purchaseProperty() {
    ...
    if (currentPlayer.money < currentSquare.value) {
      ...
      if (currentSquare.group === "station") {
        let stations =
          currentPlayer.checkCompleteGroupOwned(currentSquare).group;
  ...
  payRentIfDue() {
    ...
    if (currentSquare.group === "tax") {
      ...
      currentPlayer.money -=
        currentSquare.rentValues[currentSquare.rentIndex] *
        this.state.moveValue;
      currentSquare.owner.money +=
        currentSquare.rentValues[currentSquare.rentIndex] *
        this.state.moveValue;
    } else {
      if (!currentSquare.owner || currentSquare.owner === currentPlayer) {
        ...
        let groupCheck =
          currentSquare.owner.checkCompleteGroupOwned(currentSquare).check;
  ...
  checkCardSquare(square) {
    ...
    if (square.group === "bonus" && square.name === "Chance") {
      ...
      if (card.text === "It is your birthday, collect £10 from each player") {
        ...
        card.text === "Pay a £10 fine (cancel) or take a Chance (OK)"
      ) {
        if (
          //confirm("Landed on " + square.name + "\n" + "\n" + card.text) == true
          false === true
        ) {
  ...
  render() {
    ...
    return (
      <div className="container-div">
        ...
        <Modal show={this.state.showNewGameModal} onHide={this.flipModalState}>
          <Modal.Header>
            ...
            <form>
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[0] = e.target.value;
                  }}
              ...
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[1] = e.target.value;
                  }}
              ...
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[2] = e.target.value;
                  }}
              ...
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[3] = e.target.value;
                  }}
              ...
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[4] = e.target.value;
                  }}
              ...
              <FormGroup>
                <FormControl
                  type="text"
                  onChange={(e) => {
                    this.playerNames[5] = e.target.value;
                  }}
