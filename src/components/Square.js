...
class Square extends React.Component {
  ...
  getClassName(index, group) {
    if ([0, 10, 20, 30].includes(index)) {
      ...
      [1, 2, 3, 4, 5, 6, 7, 8, 9].includes(index) &&
      !["bonus", "tax", "station", "utility"].includes(group)
    ) {
      return " top";
    } else if (
      [21, 22, 23, 24, 25, 26, 27, 28, 29].includes(index) &&
      !["bonus", "tax", "station", "utility"].includes(group)
    ) {
      return " bottom";
    } else if (
      [11, 12, 13, 14, 15, 16, 17, 18, 19].includes(index) &&
      !["bonus", "tax", "station", "utility"].includes(group)
    ) {
      return " right";
    } else if (
      [31, 32, 33, 34, 35, 36, 37, 38, 39].includes(index) &&
      !["bonus", "tax", "station", "utility"].includes(group)
    ) {
      return " left";
    } else {
